...
uint64_t hll_estimate(const HyperLogLog *hll) {
  ...
  if (raw <= 2.5 * HLL_M) {
    if (zero_registers > 0) {
      return (uint64_t)((double)HLL_M *
                        log((double)HLL_M / (double)zero_registers));
    }
    return (uint64_t)raw;
  }
  if (raw < (1ULL << 32) / 30.0)
    return (uint64_t)raw;
  return (uint64_t)(-(double)(1ULL << 32) *
                    log(1.0 - raw / (double)(1ULL << 32)));
}
...
int main(void) {
  ...
  double error = fabs((double)((int64_t)estimate - unique_count)) /
                 (double)unique_count * 100.0;
